<template>
    <div class="rating_page">
        <top-header headTitle="我的优惠" goBack="true"></top-header>
        <section v-if="!showLoading">
            <section class="category_title">
                <span :class="{chosed:categoryType == 1}" @click="categoryType = 1">红包</span>
                <span :class="{chosed:categoryType == 2}" @click="categoryType = 2">商家代金券</span>
            </section>
            <transition name="router-slid">
                <section v-if="categoryType == 1">
                    <section class="redpocket_container">
                        <header class="redpocket_header">
                            <section class="total_number">
                                有<span>{{redpocketList.length}}</span>个红包即将到期
                            </section>
                            <section class="redpocket_description">
                                <img src="../../images/description.png" height="24" width="24">
                                <router-link to="/benefit/hbDescription" class="redpocket_detail">红包说明</router-link>
                            </section>
                        </header>
                        <ul class="redpocket_ul_list">
                            <li class="redpocket_li_list" v-for="item in redpocketList" :key="item.id">
                    ...
                    <router-link to="/benefit/hbHistory" class="history_redpocket">
                        <span class="check_history">查看历史红包</span>
                        <svg class="history_right">
                            <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#arrow-right"></use>
                        </svg>
                    </router-link>
                    <footer class="redpocket_footer">
            ...
            <transition name="router-slid">
                <section v-if="categoryType == 2" class="voucher_container">
                    <section class="redpocket_description voucher_header">
                        <img src="../../images/description.png" height="24" width="24">
                        <router-link to="/benefit/coupon" class="redpocket_detail">商家代金券说明</router-link>
                    </section>
...
import {getRedpocket} from '@/service/getData';
...
export default {
    ...
    methods:{
        async initData(){
            if (this.userInfo) {
                this.redpocketList = (await getRedpocket(this.userInfo.user_id)).data;
