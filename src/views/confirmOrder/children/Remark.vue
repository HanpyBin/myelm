<template>
    <div class="common_page">
        <section v-if="!showLoading">
            <top-header headTitle="订单备注" goBack="true"></top-header>
            <section class="quick_remark" v-if="remarkList.remarks.length">
                ...
                <ul class="remark_arr_list_ul">
                    <li class="remark_arr_list_li" v-for="(item, index) in remarkList.remarks" :key="index">
                        <span v-for="(remarkItem, remarkIndex) in item" :key="remarkIndex" class="remark_item_li" :class="{first: remarkIndex==0, last: remarkIndex == item.length-1, chosed:remarkText[index]&&(remarkText[index][0] == remarkIndex)}" @click="chooseRemark(index, remarkIndex, remarkItem)">{{remarkItem}}</span>
                    </li>
                </ul>
            </section>
...
export default {
    ...
    methods:{
        ...
        async initData(){
            this.remarkList = (await getRemark(this.id, this.sig)).data;
